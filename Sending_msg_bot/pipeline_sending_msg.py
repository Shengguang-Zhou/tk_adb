...
def sleep_time(type):
    if type == 'short':
        sleepy_time = random.uniform(0.1,0.5)
    if type == 'medium':
        sleepy_time = random.uniform(0.5,1.5)
    if type == 'long':
        sleepy_time = random.uniform(1.5,3.0)
    if type == 'extra':
        sleepy_time = random.uniform(3,5)
    if type == 'extreme':
        sleepy_time = random.uniform(8,10)
...
def send_private_msg(users:list,name):
    ...
    for user in users:
        ...
        sleep_time('extra')
        # 取消关注
        send_msg_TikTok_action.unfollow()
        sleep_time('extra')
        # 回到搜索页
        send_msg_TikTok_action.back()
        sleep_time('extra')
